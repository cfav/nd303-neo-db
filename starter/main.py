...
- Find N NEOs between start_date and end_date. For example:
main.py display --return NEO -n 5 --start_date 2020-01-01 --end_date 2020-01-10
- Find N NEOs between start_date and end_date with filters and output to
csvfile with name 'neo_neo_data' For example:
`csvfile -n 10 -f new_neo_data --start_date 2020-01-01 --end_date 2020-01-10 \
--filter "is_hazardous:=:False" "diameter:>:0.02" "distance:>=:50000`

Output options: Required.
- display: prints to stdout
- csv_file: exports data to a csv

Filters options: Optional. Input as: option:operation:value . For example:
- diameter:>=:0.042
- is_hazardous:[=]:bool
- diameter:[>=|=|<=]:float
- distance:[>=|=|<=]:float
...
Filename: Optional, used for specifying a filename for a csv to load data from.
By default project looks for a csv in: data/neo_data.csv.
"""
...
def verify_date(datetime_str):
    """
    Function that validates datetime strings in YYYY-MM-DD format
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f'File {var_args.get("filename")} not found')
        sys.exit()
